...
Find normal command in grub\_command\_list and involve callback function grub\_cmd\_normal.
...
In grub\_cmd\_normal, autoload config lists in dedicated directory, get grub configuration from specify directory in first partition, enter normal mode. In normal mode, first read lists and auto-loading, then read configuration information from grub.cfg, show grub menu and wait for response from end user.
...
Before execute grub\_commad\_execute \("boot", 0, 0\), print all loaded modules as follow:

```loaded\_modules\_before\_boot
(gdb) print_all_modules 
